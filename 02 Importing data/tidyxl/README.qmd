...
The next columns are related to the result of `xlsx_formats` which is a nested list of formatting definitions from spreadsheets. With the next code, we can print a tree with second level groups present in the list. For example, `cell-format$local$alignment` doesn't have any other list bellow 
...
- **style_format**: An index into a table of style formats `x$formats$style`.

![](img/07-excel-styles.png)

- **local_format_id**: An index into a table of style formats `x$formats$local`.
